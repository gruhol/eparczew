...
export class CartComponent implements OnInit {
  ...
  ngOnInit(): void {
    let id = Number(this.route.snapshot.queryParams['idProduct']);
    if (id > 0) {
      this.addToCart(id);
    } else {
      this.getCart();
  ...
  addToCart(id: number) {
    let cartId = Number(this.cookieService.get("cartId"));
    this.cartService.addToCart(cartId, {idProduct: id, quantity: 1})
      .subscribe(summary => {
        this.summary = summary;
        this.patchFormItems();
        this.cartIconService.cartChanged(summary.items.length);
  ...
  patchFormItems() {
    let formItem = <FormArray>this.formGroup.get("items");
    this.summary.items.forEach(item => {
      formItem.push(this.formBuilder.group({
        id: [item.id],
  ...
  mapToRequestListDto(): any {
    ...
    return items.map(items => ({
      idProduct: items.product.idProduct,
      quantity: items.quantity
    }))
  }

  deleteItem(itemId: number) {
    this.cartService.deleteCartItem(itemId).subscribe(() => this.ngOnInit());
  }
